...
export function MapView({ atolls, islandsByAtoll, groupId }: MapViewProps) {
  ...
  const islands = selectedAtoll ? (islandsByAtoll[selectedAtoll] ?? []) : [];

  const handleSelectAtoll = (atollId: string) => {
  ...
  const loadIslandData = async (islandId: string) => {
    try {
      ...
      setLocations(locs);
      setHeatMapPoints(
        heat.map((p) => [p.Latitude, p.Longitude, p.Weight] as [number, number, number])
      );
    } catch {
      setAddresses([]);
      setLocations([]);
  ...
  const totalVoters = addresses.reduce((sum, a) => sum + a.Count, 0);
  const pinnedCount = addresses.filter((a) => a.Latitude !== null).length;
      ...
      {selectedIsland && (
        <Tabs defaultValue="addresses">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <CardTitle className="text-base">Address List</CardTitle>
                  {selectedAddress && (
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead className="text-right w-20">Voters</TableHead>
                          ...
                          <TableBody>
                            {addresses.map((addr) => {
                              const isPinned = addr.Latitude !== null;
                              return (
                                <TableRow
                                ...
                                >
                                  ...
                                  <TableCell className="text-right font-medium">{addr.Count}</TableCell>
                                  <TableCell>
                                    {isPinned ? (
          ...
          <TabsContent value="heatmap" className="mt-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Voter Density</CardTitle>
              </CardHeader>
              <CardContent>
                {heatMapPoints.length === 0 ? (
                  <EmptyState
                    icon={Flame}
                    title="No heat map data"
                    description="Pin address locations in Settings to see voter density."
                  />
                ) : (
                  <AddressMap
                    locations={[]}
